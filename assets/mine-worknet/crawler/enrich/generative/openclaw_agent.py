...
_CLI_NOISE_PATTERNS = (
    re.compile(r"^\s*\[agents/auth-profiles\].*$", re.IGNORECASE),
    re.compile(r"^\s*\[model-fallback/decision\].*$", re.IGNORECASE),
...
def _extract_content(text: str) -> str:
    ...
    if data is None:
        tail_json = _extract_last_json_object(text)
        if tail_json:
            try:
                data = json.loads(tail_json)
            except json.JSONDecodeError:
                data = None
    ...
    return _strip_cli_noise_lines(text).strip()


def _extract_content_from_payload(data: dict[str, Any]) -> str:
    ...
    # Some models reply with the target JSON object directly.
    if any(key in data for key in ("result", "score", "reason", "breakdown")):
        try:
...
def _strip_cli_noise_lines(text: str) -> str:
    ...
    kept: list[str] = []
    for line in lines:
        if any(pattern.match(line) for pattern in _CLI_NOISE_PATTERNS):
            continue
        kept.append(line)
    return "\n".join(kept)
