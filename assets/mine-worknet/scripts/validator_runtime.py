...
def _payload_string_value(payload: Any, *keys: str) -> str:
    """Return the first non-empty string-like value from a dict payload."""
...
class ValidatorRuntime:
    ...
    def _poll_evaluation_task_http(self) -> None:
        ...
        if not eligible or paused:
            return
        try:
            claim_data = self._claim_evaluation_task_payload(context="HTTP polling")
            if not claim_data:
                return
    ...
    def _claim_evaluation_task_payload(self, *, context: str) -> dict[str, Any] | None:
        with self._platform_lock:
        ...
        if isinstance(claim_data, dict) and claim_data.get("_cooldown"):
            ...
            log.info("Validator cooldown via %s: sleeping %ds", context, retry_after)
            self._stop_event.wait(timeout=retry_after)
            return None
        return claim_data if isinstance(claim_data, dict) else None
    ...
    def _handle_evaluation_task(self, msg: WSMessage, *, via_http: bool = False) -> None:
        ...
        HTTP polling flow:
          ...
          2. 评估 + HTTP report
        """
        claim_data = dict(msg.data) if isinstance(msg.data, dict) else {}
        assignment_id = msg.assignment_id or _payload_string_value(
            claim_data, "assignment_id", "assignmentId", "assignment"
        )
        task_id = msg.task_id or _payload_string_value(
            claim_data, "task_id", "taskId", "id", "task"
        )
        ws_task_id = task_id

        if not via_http and not assignment_id:
            log.warning(
                "WS task missing assignment_id for task=%s; switching to HTTP claim fallback",
                task_id or "<unknown>",
            )
            claimed_task = self._claim_evaluation_task_payload(context="WS fallback")
            if not claimed_task:
                return
            claim_data = dict(claimed_task)
            assignment_id = _payload_string_value(
                claim_data, "assignment_id", "assignmentId", "assignment"
            )
            task_id = _payload_string_value(
                claim_data, "task_id", "taskId", "id", "task"
            )
            via_http = True
            if ws_task_id and task_id and task_id != ws_task_id:
                log.info(
                    "HTTP fallback claimed task=%s after WS task=%s arrived without assignment_id",
                    task_id,
                    ws_task_id,
                )
        ...
        if not via_http:
            if assignment_id:
                ...
                log.warning(
                    "Skipping ack_eval because assignment_id is missing for task=%s",
                    task_id or "<unknown>",
                )
        ...
        if not via_http:
            try:
                claimed_task = self._claim_evaluation_task_payload(context="task detail claim")
                if isinstance(claimed_task, dict) and claimed_task:
                    claimed_task_id = _payload_string_value(
                        claimed_task, "task_id", "taskId", "id", "task"
                    )
                    if task_id and claimed_task_id and claimed_task_id != task_id:
                        log.warning(
                            "Claim endpoint returned task=%s while WS advertised task=%s; using claimed task payload",
                            claimed_task_id,
                            task_id,
                        )
                    claim_data = {**claim_data, **claimed_task}
                    task_id = claimed_task_id or task_id
                    assignment_id = assignment_id or _payload_string_value(
                        claim_data, "assignment_id", "assignmentId", "assignment"
                    )
            except Exception as exc:
                log.warning("HTTP claim for task %s failed: %s", task_id or "<unknown>", exc)
        ...
        if not cleaned_data or not structured_data:
            try:
                ...
                if isinstance(task_detail, dict):
                    task_id = task_id or _payload_string_value(
                        task_detail, "task_id", "taskId", "id", "task"
                    )
                    assignment_id = assignment_id or _payload_string_value(
                        task_detail, "assignment_id", "assignmentId", "assignment"
                    )
        ...
        if not task_id:
            raise RuntimeError("Missing task_id in evaluation task payload")
        if not assignment_id:
            raise RuntimeError(f"Missing assignment_id for task {task_id}")
    ...
    def _heartbeat_loop(self) -> None:
        ...
        WS 连接时仍然定期发 HTTP 心跳（每个周期发一次）——仍需从
