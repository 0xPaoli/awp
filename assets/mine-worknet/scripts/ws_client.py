...
def _payload_string_value(mapping: dict[str, Any], *keys: str) -> str:
...
def _normalize_ws_payload(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate = {key: value for key, value in raw.items() if key != "type"}
    if any(
        key in candidate
        for key in ("task_id", "taskId", "assignment_id", "assignmentId", "submission_id", "submissionId", "id")
    ):
        return candidate
    return {}
...
class WSMessage:
    ...
    def __init__(self, raw: dict[str, Any]) -> None:
        ...
        self.data: dict[str, Any] = _normalize_ws_payload(raw)

    @property
    def task_id(self) -> str:
        return (
            _payload_string_value(self.data, "task_id", "taskId", "id", "task")
            or _payload_string_value(self.raw, "task_id", "taskId", "id", "task")
        )

    @property
    def assignment_id(self) -> str:
        return (
            _payload_string_value(self.data, "assignment_id", "assignmentId", "assignment")
            or _payload_string_value(self.raw, "assignment_id", "assignmentId", "assignment")
        )

    @property
    def submission_id(self) -> str:
        return (
            _payload_string_value(self.data, "submission_id", "submissionId", "submission")
            or _payload_string_value(self.raw, "submission_id", "submissionId", "submission")
        )

    @property
    def mode(self) -> str:
        return str(self.data.get("mode") or self.raw.get("mode") or "single")

    @property
    def repeat_crawl_task_id(self) -> str:
        """Task ID from repeat_crawl_task message."""
        return (
            _payload_string_value(self.data, "id", "task_id", "taskId")
            or _payload_string_value(self.raw, "id", "task_id", "taskId")
        )
